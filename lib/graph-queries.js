...
function itemValidations(graph, id) {
    const result = [];
    forEachOutgoingNode(graph, 'item:' + id, function(toNode, link) {
        if (toNode.data.type === 'source' && link.data.type === 'validation') {
            result.push({ source: toNode.data, location: link.data.location });
        }
    });
    return result;
}
